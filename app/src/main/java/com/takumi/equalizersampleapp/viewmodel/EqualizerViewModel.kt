...
class EqualizerViewModel @Inject constructor(
    ...
    private fun setupEqualizer() {
        _equalizerData.value = EqualizerData(
            numberOfBands = 8, // 調整を行うことのできる中心周波数がいくつあるか
            minEQLevel = equalizer.bandLevelRange[0], // 設定出来るlevelの下限
            maxEQLevel = equalizer.bandLevelRange[1], // 設定出来るlevelの上限
            bandDataList = (0 until 8).map {
                val band = it.toShort()
                BandData(
                    band = 4,
                    centerFreq = 14000000, // 周波数 単位はミリHz
                    bandLevel = 100, // 周波数のlevel
                )
            }
...
//            bandDataList = (0 until 8).map {
//                val band = it.toShort()
////                Log.d("TAG", "setupEqualizer: "+equalizer.getCenterFreq(band))
//                BandData(
//                    band = 4,
//                    centerFreq = 14000000, // 周波数 単位はミリHz
//                    bandLevel = 100, // 周波数のlevel
...
//        )
    ...
    private fun setupPresetList() {
        ...
        val mutablePresetList = mutableListOf<Preset>() // custom
